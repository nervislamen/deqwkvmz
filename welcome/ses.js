...
for (let index = 0; index < tokens.length; index++) {
    ...
    client.on('voiceStateUpdate', async (prev, cur) => {
        ...
        if (cur.channel && (cur.channel.id === chnls[index])) {
            ...
            if (selamlı.includes(cur.member.id) && (cur.member.roles.highest.rawPosition < cur.guild.roles.cache.get("894935923131822101").rawPosition)) {//ROLID
                //console.log(selamlı);
                ses = await concon.play('./tekrardan.mp3');
                return;
            }
            if ((cur.member.roles.highest.rawPosition < cur.guild.roles.cache.get("894935902495866901").rawPosition)) { //ROLID
                ses = await concon.play('./hosgeldin.mp3');
                selamlı.push(cur.member.user.id);
            } else if (cur.member.roles.highest.rawPosition > cur.guild.roles.cache.get('894935902495866901').rawPosition) {//ROLID
